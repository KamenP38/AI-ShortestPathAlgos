...
class AStar:
    ...
    def aStar(self, initState, finalState, drivingInfo, straightInfo):
    ...
    def expand(self, currName, goal, drivingInfo, straightInfo):
        ...
        for i in range(len(data[currName].values)):
            if(int(data[currName].values[i]) != -1 and data['STATE'].values[i] != currName):
                # driving distance => remove the straight line distance from previous node
                toRemove = self.removeStraightDist(currName, goal, straightInfo)
                prevNodeTotal = self.reached[currName][1] - toRemove
                drivingDistFromPrev = data[currName].values[i]
                
                # currCostTotal = straight line distance to goal from new state + distance to previous state + driving distance from previous state to initial state
                # by using the path that we have fo this previous state so far
                currCostTotal = data2[goal].values[i] + drivingDistFromPrev + prevNodeTotal
                ...
                if(data['STATE'].values[i] not in self.reached):
                    self.reached.update({data2['STATE'].values[i]: [currName , currCostTotal]})
                    self.frontier.update({data2['STATE'].values[i]: currCostTotal})
                ...
                elif(data['STATE'].values[i] in self.reached) and (currCostTotal < self.reached[data['STATE'].values[i]][1]):
                    self.reached[data['STATE'].values[i]][1] = currCostTotal
                    self.reached[data['STATE'].values[i]][0] = currName
                    self.frontier.update({data2['STATE'].values[i]: currCostTotal})
    
    # I used this function to get rid of the straight line distance of the previous state/node 
    # so I can use its driving cost to do calculations in the "expand" function
    def removeStraightDist(self, curr, goal, straightInfo):
        costToRemove = 0
        data2 = pd.read_csv(straightInfo, usecols = ['STATE', goal])
        for i in range(len(data2[goal].values)):
            if(data2['STATE'].values[i] == curr):
                costToRemove = data2[goal].values[i]
        return costToRemove
    ...
    def calculateTime(self, initial, goal, drivingInfo, straightInfo):
        ...
        a = self.aStar(initial, goal, drivingInfo, straightInfo)
